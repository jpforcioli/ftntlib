...
class FortiManagerJSON(object):
    ...
    def login(self, ip, user, passwd, ssl=True):
        ...
        params = [
            {"url": "/sys/login/user", "data": [{"passwd": passwd, "user": user}]}
        ]
    ...
    def _do(self, method, url, data={}):
        store = {"root": self._root, "skip": self._skip, "verbose": self._verbose}
        self._root = False
        self._skip = False
        self._verbose = False
    ...
    def install_package(self, adom, package, scope, flags=["install_chg"]):
        ...
        data = {"adom": adom, "pkg": pkg, "flags": flags, "scope": scope}
    ...
    def discover_device(self, ip, username, password):
        ...
        deviceinfo = {"ip": ip, "adm_usr": username, "adm_pass": password}
    ...
    def add_device(self, adom, ip, username, password, mgmtmode=3):
        ...
        data = {"adom": adom, "device": device, "flags": ["create_task", "nonblocking"]}
        status, response = self._do("exec", url, data)
